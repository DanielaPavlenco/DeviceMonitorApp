...
class PackagesManager(private val context: Context) {
        ...
        packageNames.forEach { packageName ->
            if (packageName.isNotEmpty()) {
                val app = App().apply {
                    ...
                    versionName = getVersionName(packageName).toString()
                    isEnabled = true
                    isSystemApp = false
                }
                apps.add(app)
    ...
    fun getAppIcon(packageName: String?): Drawable? {
        return try {
            packageName?.let { packageManager.getApplicationIcon(it) }
        } catch (e: Exception) {
    ...
    fun getAppLabel(packageName: String?): String {
        return try {
            val applicationInfo: ApplicationInfo? = packageName?.let { packageManager.getApplicationInfo(it, 0) }
    ...
    fun getVersionName(packageName: String?): String? {
        return runCatching {
            packageName?.let {
                val info = packageManager.getPackageInfo(it, 0)
